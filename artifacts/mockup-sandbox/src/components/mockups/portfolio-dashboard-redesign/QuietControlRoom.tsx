...
type Section = "dashboard" | "updated" | "projects" | "experience" | "homelab" | "skills" | "contact";
...
const nav: Array<[Section, string]> = [["dashboard", "Dashboard"], ["updated", "Last updated"], ["projects", "Projects"], ["experience", "Work experience"], ["homelab", "Homelab"], ["skills", "Skills"], ["contact", "Contact"]];
...
function QuietControlRoom() {
  ...
  const [lastUpdated, setLastUpdated] = useState("2026-09-19");
  const [updateNote, setUpdateNote] = useState("Refined the dashboard and homelab record.");
  const [postedUpdate, setPostedUpdate] = useState({ date: "2026-09-19", note: "Refined the dashboard and homelab record." });
    ...
    <div className="qcr">
      ...
      <main className="qcr-main">
        ...
        <section id="dashboard" className="qcr-hero"><div className="qcr-eyebrow"><span><span className="qcr-lamp" /> system / maintained record</span><span>Chicago, IL · last reviewed by Torianna</span></div><div className="qcr-hero-grid"><div><p className="qcr-kicker">Security operations / portfolio dashboard</p><h1>Homelab<br /><em>Dashboard</em></h1><p className="qcr-intro">I'm Torianna, a cybersecurity graduate who learns best through hands-on work. This site brings together my work experience, past and current projects, and everything I'm currently building in my homelab. Thanks for visiting!</p><div className="qcr-actions"><button className="qcr-primary" onClick={() => go("projects")} data-testid="button-inspect-projects">Inspect projects <ChevronRight size={16} /></button><button className="qcr-quiet" onClick={() => go("homelab")} data-testid="button-open-homelab">Open homelab</button></div></div><div className="qcr-profile"><div className="qcr-profile-head"><span>/ operator profile</span><Terminal size={16} /></div><dl><dt>focus</dt><dd>defensive security</dd><dt>detection</dt><dd className="green"><span className="qcr-lamp" /> lab active</dd><dt>response loop</dt><dd>in progress</dd><dt>host</dt><dd>Ubuntu / dedicated</dd></dl><p className="qcr-note"><b>note</b> no live attack telemetry</p></div></div><div className="qcr-metrics"><div><b>03</b><span>projects complete</span></div><div><b>01</b><span>certification earned</span></div><div><b>04</b><span>work records</span></div><div><b>06</b><span>lab milestones</span></div></div></section>
         <section id="updated" className="qcr-section qcr-updated"><div className="qcr-section-head"><div><p className="qcr-kicker">Workspace log</p><h2>Last updated</h2><p>Keep a small public note for the latest change to the portfolio. Edit the date and note, then post it to this dashboard record.</p></div><span className="qcr-stamp">editable local record</span></div><div className="qcr-update-grid"><div className="qcr-update-current"><div className="qcr-card-title"><span>Current record</span><span className="qcr-status"><span className="qcr-lamp" /> posted</span></div><strong>{postedUpdate.date}</strong><p>{postedUpdate.note}</p><small>Last updated entry</small></div><form className="qcr-update-form" onSubmit={event => { event.preventDefault(); setPostedUpdate({ date: lastUpdated, note: updateNote }); }}><label htmlFor="qcr-update-date">Date<input id="qcr-update-date" type="date" value={lastUpdated} onChange={event => setLastUpdated(event.target.value)} /></label><label htmlFor="qcr-update-note">What changed?<input id="qcr-update-note" type="text" value={updateNote} onChange={event => setUpdateNote(event.target.value)} /></label><button type="submit" className="qcr-primary" data-testid="button-post-update">Post update <ChevronRight size={16} /></button></form></div></section>
